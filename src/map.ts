...
map.on("click", function(e: any) {
    if (!leavingMarker) {
        ...
        leavingMarker.on('dragend', function(event: any) {
        ...
        arrivedMarker.on('dragend', function(event: any) {
            let marker = event.target;
            let position = marker.getLatLng();
...
function updateInputWithLocationName(latlng:any, inputId:any) {
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latlng.lat}&lon=${latlng.lng}`)
        .then(response => response.json())
        .then(data => {
            if (inputId === "departurePoint") {
                const country = data.address.country || `${latlng.lat}, ${latlng.lng}`;
                const departPoint = document.getElementById(inputId) as HTMLInputElement;
                if(departPoint){
                    departPoint.value = country;
                }
            } else if (inputId === "arrivalPoint") {
                const country = data.address.country || `${latlng.lat}, ${latlng.lng}`;
                const arrivalPoint = document.getElementById(inputId) as HTMLInputElement;
                if(arrivalPoint){
