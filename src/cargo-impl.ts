...
function pagination(page: number = currentPage){
    fetch('api.php?action=getCargaison', {
      ...
      // Ajoutez des écouteurs d'événements aux boutons
       ...
       // Ajoutez des écouteurs d'événements aux boutons "Détails"
document.querySelectorAll('.show-details-btn').forEach(button => {
    button.addEventListener('click', (event) => {
        const cargaisonId = (event.target as HTMLButtonElement).getAttribute('data-id');

        console.log(cargaisonId);
                                (document.getElementById('detailsModal') as HTMLDialogElement).style.display='';

        showDetails(Number(cargaisonId),cargaisons);
    });
});

function showDetails(cargaisonId: number,cargaison:Cargo[]) {
    console.log(cargaison,cargaisonId);
    let cargaisonD:any;
    cargaison.forEach(cargo => {
    ...
    let details = `
    <p><strong>Code:</strong> ${cargaisonD!.reference}</p>
    <p><strong>Type:</strong> ${cargaisonD!.id}</p>
    <p><strong>Distance:</strong> ${cargaisonD!.distance} km</p>
    <p><strong>Lieu de départ:</strong> ${cargaisonD!.departurePoint}</p>
    ...
    //affiche info produit
    const prod = document.getElementById('tbody-details');
    cargaisonD!.products!.forEach(product => {
   ...
   /*  if (cargaisonD) {
        ...
        if (cargaisonD.products && cargaisonD.products.length > 0) {
            details += '<p><strong>Produits:</strong></p><ul>';
            cargaisonD.products.forEach((product, index) => {
                details += `<li>${index + 1}. Libellé - ${product.libelle}, Type - ${product.constructor.name}</li>`;
            });
            details += '</ul>';
        } else {
            details += '<p>Aucun produit associé à cette cargaison.</p>';
        }
